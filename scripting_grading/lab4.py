...
key = dict(
    q1 = dict(entrytype='cell', pts=4, auto=True, answers=L4_Q1_Answers,
              question="Write the code to return the first 7 rows of `bodf`"),
    q2 = dict(entrytype='var', pts=4, auto=True, answers=['254060295'],
              question="What is the average box office take in this sample of films, to the nearest dollar?"),
    q3 = dict(entrytype='cell', pts=4, auto=True, answers=L4_Q3_Answers,
              question="Write the code to return just the movies that have made more than $250m."),
    q4 = dict(entrytype='var', pts=4, auto=False, question="What does bodf.cumsum() do?"),
    q5 = dict(entrytype='cell', pts=4, auto=True, answers=L4_Q5_Answers,
              question="How would you return the Box Office take for Dances with Wolves?"),
    q6 = dict(entrytype='var', pts=4, auto=True, answers=["ORDER BY"],
              question="What SQL clause is comparable to bodf.sort_values()?"),
    q7 = dict(entrytype='cell', pts=4, auto=True, answers=L4_Q7_Answers,
              question="How would you sort bodf in descending order?"),
    q8 = dict(entrytype='cell', pts=4, auto=True, answers=L4_Q8_Answers,
              question="What is the code to select rows 5 to 10 of mpg?"),
    q9 = dict(entrytype='cell', pts=5, auto=True, answers=L4_Q9_Answers,
              question="How would you select all rows with a class of 'midsize'?"),
    q10 = dict(entrytype='cell', pts=5, auto=True, answers=L4_Q10_Answers,
              question="How would you select all rows with a class of 'midsize' and hwy economy of 24 miles per gallon?"),
    q11 = dict(entrytype='var', pts=5, auto=True, answers=[('20.1', 'max', ''), ('23.4', '-2', 'That is the mean for all the cars.')],
              question="What is the average city fuel economy for compact cars?"),
    q12 = dict(entrytype='cell', pts=7, auto=True, answers=L4_Q12_Answers,
              question="Convert the SQL to the equivalent in Pandas."),
    q13 = dict(entrytype='var', pts=6, auto=True, answers=[('Volkswagen New Beetle', 'max', ''), ("Toyota Corolla", '2', 'Did you remember to specify 1999? This is the top in the >2005 subset of the previous question...')],
    ...
    q16a = dict(entrytype='var', pts=2, auto=True, answers=["pd.read_csv()"], question="Method"),
    q16b = dict(entrytype='var', pts=2, auto=True, answers=['3.1415'], question="Float"),
    q16c = dict(entrytype='var', pts=2, auto=True, answers=["Python"], question="Programming Language"),
    q16d = dict(entrytype='var', pts=2, auto=True, answers=["SELECT * FROM heights;"], question="SQL Statement"),
    q16e = dict(entrytype='var', pts=2, auto=True, answers=[">"], question="Logical Operator"),
    q16f = dict(entrytype='var', pts=2, auto=True, answers=["[1, 2, 3]", "1,2,3"], question="List"),
    q16g = dict(entrytype='var', pts=2, auto=True, answers=["sep=' ', as in print(\"hello world\", sep=' ')"], question="Named Argument"),
    q16h = dict(entrytype='var', pts=2, auto=True, answers=["42"], question="Integer"),
    q16i = dict(entrytype='var', pts=2, auto=True, answers=["WHERE"], question="SQL Clause"),
    q16j = dict(entrytype='var', pts=2, auto=True, answers=["_"], question="LIKE wildcard"),
    q16k = dict(entrytype='var', pts=2, auto=True, answers=["SQLite"], question="DBMS"),
    q16l = dict(entrytype='var', pts=2, auto=True, answers=["print()"], question="Function"),
    q16m = dict(entrytype='var', pts=2, auto=True, answers=["Pandas"], question="Library"),
)