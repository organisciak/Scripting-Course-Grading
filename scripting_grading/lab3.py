...
key = dict(
    ...
    q5 = dict(entrytype='var', pts=4, auto=True, question="How many records are listed in books with George Orwell as the author?"),
    q6a = dict(entrytype='var', pts=4, auto=True, question="Written by an author with the middle initial C, like in Arthur C. Clarke: WHERE author LIKE ...."),
    q6b = dict(entrytype='var', pts=4, auto=True, question="Title starting with the word The:  WHERE title LIKE ..."),
    q6c = dict(entrytype='var', pts=5, auto=True, question="Written by authors named like Tim or Tom (starts with a T, three characters, ends with an m): WHERE author LIKE ..."),
    ...
    q13a = dict(entrytype='var', pts=3, auto=True, question="What type of JOIN is necessary to retrieve all the comments, with user names and images if available?"),
    q13b = dict(entrytype='var', pts=3, auto=True, question="What type of JOIN is necessary to retrieve all the comments by active users, with user names included?"),
    q13c = dict(entrytype='var', pts=4, auto=True, question="What type of JOIN is necessary to retrieve all the active and deleted comments by a user?"),
